...
<template>
    <div ref="windowRef" class="window" @pointerup="touchEnd" @pointermove="touchMove" @pointerdown="touchStart"
...
const getSwiperStyles = () => {
    return {
        ...
        '--list-height': `${props.slideNum * 100}%`
    }
}
// 翻页函数 传参为下标与滑动行为
...
const touchStart = (e) => {
    ...
    move.isDown = true
    move.startY = move.curY = e.clientY
}
// 触摸移动函数
const touchMove = (e) => {
    if (!moveData.value.isDown) return
    const move = moveData.value
    const gap = e.clientY - move.curY
    if (gap > 0 && move.curIdx == 1)
        return
    if (gap < 0 && move.curIdx == props.slideNum)
        return
    windowRef.value.scrollTop -= gap
    move.curY = e.clientY
}
// 触摸结束函数
const touchEnd = () => {
    ...
    const move = moveData.value
    const finalGap = move.startY - move.curY
    move.upTime = performance.now()

    if (finalGap / windowRef.value.offsetHeight > 0.2) {
